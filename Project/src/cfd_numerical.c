...
// Down boundary - Left - Right
  for(int i = 0; i < (*Problem).Nx ; i++ ){
    if   (i < (*Problem).NLs ){ (*Problem).omega[i][(*Problem).NHs-1] = -3.0/((*Problem).h*(*Problem).h) * (*Problem).psi[i][(*Problem).NHs-1] - 0.5*(*Problem).omega[i][(*Problem).NHs-1]; }
                                              // pas plutot ?         = -3.0/((*Problem).h*(*Problem).h) * (*Problem).psi[i][(*Problem).NHs] - 0.5*(*Problem).omega[i][(*Problem).NHs];
    else                        (*Problem).omega[i][(*Problem).Ny -1] = -3.0/((*Problem).h*(*Problem).h) * (*Problem).psi[i][(*Problem).Ny -1] - 0.5*(*Problem).omega[i][(*Problem).Ny -1];
  }
// Down boundary Side
